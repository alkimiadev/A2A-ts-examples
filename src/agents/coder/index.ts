...
import { CodeMessage } from "./code-format.js";
...
export async function* coderAgent({
  ...
  if (messages.length === 0) {
    console.error(`[CoderAgent WARN] No history/messages found for task ${task.id}`); // Use console.error
    yield {
      state: "failed",
      message: {
        role: "agent",
    ...
    for (const fileUpdate of codeChunk.files) {
      ...
      if (!fileOrder.includes(fileUpdate.filename)) {
        ...
        if (
          ...
          console.error( // Use console.error
  ...
  for (let i = emittedFileCount; i < fileOrder.length; i++) {
    ...
    console.error(`[CoderAgent] Emitting final file (index ${i}): ${filename}`); // Use console.error
    yield {
      index: i,
      name: filename,
...
if (useStdio) {
  ...
  console.error("[CoderAgent] Starting Stdio server...");
  const stdioServer = new A2AStdioServer(coderAgent); // Defaults to InMemoryTaskStore
  stdioServer.start();
  console.error("[CoderAgent] Stdio server started, listening on stdin.");
  console.error("[CoderAgent] Press Ctrl+C or close stdin to stop the server"); // Ensure this goes to stderr

} else {
  // Start HTTP Server (Default) - Keep using console.log for HTTP mode
  console.log("[CoderAgent] Starting HTTP server...");
  const httpServer = new A2AServer(coderAgent, { card: coderAgentCard });
  const port = 41241; // Or get from env/args if needed
  httpServer.start(port);
  console.log(`[CoderAgent] HTTP Server started on http://localhost:${port}`); // Keep console.log for HTTP
  console.log("[CoderAgent] Press Ctrl+C to stop the server"); // Keep console.log for HTTP
}
